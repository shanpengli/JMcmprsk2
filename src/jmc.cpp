 ...
 void STAT(gsl_matrix *store,int i,double *mean,double *sd)
{
 ...
 int EM(
    ...
    const gsl_matrix *Y,
    const gsl_matrix *C,
    const gsl_matrix *C_new,
    const gsl_permutation *rank,//add C_new in EM process for risk set calculation
    const gsl_vector *M1,
    const int p1a,
    const int maxl,
	const int point,
...
)
{
  ...
  gsl_matrix *FUNW=gsl_matrix_alloc(maxl,k),
             ...
             *FUNWBS=gsl_matrix_alloc(maxl*p1a*(p1a+1)/2,k);
             //define new matrix
  gsl_matrix *FUNE_new = gsl_matrix_alloc(2,k);
  gsl_vector *FU = gsl_vector_alloc(2);
      int status;
      status = GetE(FUNU,FUNUS,FUNB,FUNBS,FUNBU,FUNE,FUNUSE,FUNUE,FUNW,FUNWB,FUNWBS,beta,gamma,vee,H01,H02,
                    *sigma,sig,Y,C,M1,p1a,maxl,point,xs,ws);
      if (status==100) return status;
      ...
      for (j=0;j<k;j++)
      {
        gsl_matrix_get_col(FU, FUNE, j);
        index1 = k - 1 - (int) gsl_permutation_get(rank, j);
        gsl_matrix_set_col(FUNE_new, index1, FU);
      }
      /* calculate beta, sig */
      gsl_matrix_set_zero(SZZ);
      gsl_vector_set_zero(SZ);
    ...
    for (j=0; j<k; j++)
    {
        ...
        for(u=0;u<p2;u++)
        {
            gsl_vector_set(X,u,gsl_matrix_get(C_new,j,2+u));
            gsl_vector_set(gammai,u,gsl_matrix_get(gamma,0,u));
        }
        dem = dem + gsl_matrix_get(FUNE_new,0,j)*exp(MulVV(X,gammai));
    ...
    for (j=0;j<k;j++)
    {
        ...
        dem = dem + gsl_matrix_get(FUNE_new,1,j)*exp(MulVV(X,gammai));
   ...
   /*
    for(p=0;p<a;p++)
    {
        ...
        for(j=0;j<k;j++)
        {
            if(gsl_matrix_get(C_new,j,0)>=gsl_matrix_get(H01,0,p))
            {
                for(u=0;u<p2;u++)
                {
                    gsl_vector_set(X,u,gsl_matrix_get(C_new,j,2+u));
    ...
    for(p=0;p<b;p++)
    {
        ...
        for(j=0;j<k;j++)
        {
            if(gsl_matrix_get(C_new,j,0)>=gsl_matrix_get(H02,0,p))
            {
                for(u=0;u<p2;u++)
                {
                    gsl_vector_set(X,u,gsl_matrix_get(C_new,j,2+u));
...
*/
      ...
      //free variables for risk set calculation
      gsl_matrix_free(FUNE_new);
      //gsl_vector_free(gammai1);
      //gsl_vector_free(gammai2);
      return 0;
}
...
Rcpp::List jmc_cmain(int k, int n1,int p1,int p2, int maxl, int p1a, int maxiterations, int point,std::vector<double> xs,  std::vector<double> ws, std::string yfile, std::string cfile, std::string mfile, int trace)
{
 ...
 // }
  ...
  gsl_matrix *C_new = gsl_matrix_alloc(k,2+p2);
  gsl_vector *p = gsl_vector_alloc(k);
  gsl_vector *pi = gsl_vector_alloc(2+p2);
  ...
  { //check user interrupt just in case
    ...
    status = EM(beta,gamma,vee,H01,H02,&sigma,sig,Y,C,C_new,rank,M1,p1a,maxl,point,xs,ws);//add C_new into EM process
  if(trace==1){
    ...
    if(status != 100)
    {
     ...
     //   fRprintf(output_F,"Variance-covariance matrix for all the parameters: \n");
      ...
      //    fRprintf(output_F,"\n");
        ...
        for (i=0;i<p1;i++)
			gsl_vector_set(vbeta,i,gsl_matrix_get(Cov,i,i));
     ...
     //   fRprintf(output_F,"Beta = \n");
      ...
      //  fRprintf(output_F,"standard error of beta = \n");
        ...
        for (i=0;i<p1;i++)
        {
      //    fRprintf(output_F,"%f     ", sqrt(gsl_vector_get(vbeta,i)));
     ...
     //   fRprintf(output_F,"Gamma = \n");
      ...
      //  fRprintf(output_F,"\n");
       ...
       // fRprintf(output_F,"\n");
      //  fRprintf(output_F,"sigma2 = %f\n", sigma);
		 sigma2_val=sigma;
       // fRprintf(output_F,"standard error of sigma2 = %f\n", sqrt(v_sigma));
